...
public class RegAssertionBuilder {
    ...
    public String getAssertions(RegistrationResponse response, String appFacetId) throws Exception {
        ByteArrayOutputStream byteout = new ByteArrayOutputStream();
        byte[] value = null;
        int length = 0;

        byteout.write(encodeInt(TagsEnum.TAG_UAFV1_REG_ASSERTION.id));
        value = getRegAssertion(response, appFacetId);
        length = value.length;
    ...
    private byte[] getRegAssertion(RegistrationResponse response, String appFacetId) throws Exception {
        ByteArrayOutputStream byteout = new ByteArrayOutputStream();
        byte[] value = null;
        int length = 0;
    ...
    private byte[] getSignedData(RegistrationResponse response, String appFacetId) throws IOException, InvalidKeySpecException, NoSuchAlgorithmException, NoSuchProviderException {
        ByteArrayOutputStream byteout = new ByteArrayOutputStream();
        byte[] value = null;
        int length = 0;

//		RegRecord regRecord = operationalParams.genAndRecord(response.header.appID);
        RegRecord regRecord = operationalParams.genAndRecord(appFacetId);
