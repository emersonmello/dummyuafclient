...
public class InitConfig {
    ...
    public void init(String aaid, byte[] attestCert, byte[] attestPrivKey,
                     OperationalParamsIntf operationalParams, StorageInterface storage) {

        this.operationalParams = operationalParams;

        byte[] certificate = ApplicationContextProvider.generateAttestationKey(false);

        if (!(new String(certificate)).isEmpty()) {
            attestCert = certificate;
        }
